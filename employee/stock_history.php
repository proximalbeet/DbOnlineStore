...
// Note: checkout proc does NOT log purchase-driven stock decrements (see CLAUDE.md).
// This page shows employee restocks only.
$rows = $dbh->query(
    "SELECT h.product_id, p.name, h.timestamps, h.old_stock, h.new_stock,
            h.details, h.employee_id
     FROM history_record h
     JOIN product p ON h.product_id = p.product_id
     WHERE h.old_stock IS NOT NULL OR h.new_stock IS NOT NULL
    ...
    <table border="1">
        ...
        <?php foreach ($rows as $r): ?>
            <tr>
                ...
                <td><?php echo $r["old_stock"]; ?></td>
                <td><?php echo $r["new_stock"]; ?></td>
                <td><?php echo $r["new_stock"] - $r["old_stock"]; ?></td>
